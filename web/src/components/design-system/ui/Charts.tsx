    ...
    <div className={cn("grid grid-cols-1 md:grid-cols-2 gap-6", className)}>
      ...
      <Card glowColor="emerald" className="p-6 space-y-4">
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <div>
            <span className="text-[10px] font-mono uppercase text-emerald-400">Telemetry Chart 01</span>
          ...
          {chartData.map((item, idx) => (
            <div key={idx} className="space-y-1.5 font-mono text-xs">
              <div className="flex justify-between text-slate-300">
                ...
                <span className="text-emerald-400 font-bold">{item.saved}% Saved</span>
              </div>
              <div className="h-3 rounded-full bg-white/5 overflow-hidden flex p-0.5 border border-white/10">
                <motion.div
                  initial={{ width: 0 }}
                  whileInView={{ width: `${item.saved}%` }}
                  viewport={{ once: true }}
                  transition={{ duration: 1, delay: idx * 0.15 }}
                  className="h-full rounded-full bg-gradient-to-r from-emerald-400 to-cyan-400 shadow-[0_0_15px_rgba(0,245,155,0.4)]"
                />
              </div>
            </div>
          ))}
      ...
      <Card glowColor="violet" className="p-6 space-y-4">
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <div>
            <span className="text-[10px] font-mono uppercase text-violet-400">Telemetry Chart 02</span>
            <h4 className="text-base font-bold text-white">Processing Latency Overhead (ms)</h4>
          </div>
          <span className="text-xs font-mono text-violet-400 font-bold bg-violet-500/10 px-2.5 py-1 rounded border border-violet-500/20">
            Sub-50ms Average
          </span>
        </div>
          ...
          {[
            ...
            <div key={idx} className="space-y-1.5">
              <div className="flex justify-between text-slate-300">
                ...
                <span className="text-violet-400 font-bold">{item.ms} ms</span>
              </div>
              <div className="h-3 rounded-full bg-white/5 overflow-hidden flex p-0.5 border border-white/10">
                <motion.div
                  initial={{ width: 0 }}
                  ...
                  viewport={{ once: true }}
                  transition={{ duration: 1, delay: idx * 0.15 }}
                  className="h-full rounded-full bg-gradient-to-r from-violet-500 to-pink-500 shadow-[0_0_15px_rgba(168,85,247,0.4)]"
                />
              </div>
            </div>
          ))}
