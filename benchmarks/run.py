...
def build_result(mode: str = "fast", iterations: int | None = None, seed: int = 20260815, dataset_path: Path = DEFAULT_DATASET,
                 phase4_6: Optional[Mapping[str, Any]] = None) -> Dict[str, Any]:
    ...
    result = {"schema_version": RESULT_SCHEMA_VERSION, "classification": "MEASURED", "llmslim_version": __version__,
              ...
              "security": security, "schema_tax": schema_tax, "phase4": run_phase4_benchmark(),
              "phase4_5": run_phase4_5_benchmark(), "phase4_6": dict(phase4_6) if phase4_6 is not None else {
                  "classification": "NOT_RUN_OPTIONAL", "reason": "Phase 4.6 requires an explicitly cached optional local model; default benchmark and pytest do not load it."},
...
def render_report(result: Mapping[str, Any]) -> str:
    ...
    lines.extend(["", "## Phase 4 tool-context results", "", "All numbers in this section are local measurements. The Phase 2 schema-tax baseline already uses compact canonical JSON, so lossless canonical serialization can legitimately save zero tokens from that baseline.", "", f"- Schema catalogs: {optimization['catalog_count']}; generated schemas: {optimization['tool_schema_count']}.", f"- Lossless optimization: {optimization['original_tokens']} → {optimization['optimized_tokens']} tokens ({optimization['reduction_ratio']:.2%}).", f"- Relevance Recall@1/@3/@5: {relevance['metrics']['recall_at_1']:.2%} / {relevance['metrics']['recall_at_3']:.2%} / {relevance['metrics']['recall_at_5']:.2%}; MRR: {relevance['metrics']['mrr']:.4g}.", f"- Selective exposure: {phase4['selective_exposure']['status']}.", "", "| top-k | Required-tool recall | Mean tokens avoided |", "| ---: | ---: | ---: |"])
    for row in phase4["selective_exposure"]["rows"]:
        lines.append(f"| {row['top_k']} | {row['required_tool_recall']:.2%} | {row['mean_tokens_avoided']:.2f} |")
    phase45 = result["phase4_5"]
    bm25 = phase45["strategies"]["bm25_v1_k1_1.2_b_0.75"]["test_policies"]["dynamic"]["metrics"]
    lines.extend(["", "## Phase 4.5 retrieval bakeoff", "", "Phase 4.5 is a repository-owned, static 100-tool / 500-query regression corpus. It is not a production-traffic claim. Its dynamic policy is conservative and currently RESEARCH_ONLY because development fixed-top-k all-required recall did not meet the release threshold.", "", f"- BM25 all-required-tool recall: {bm25['all_required_tool_recall']['mean']:.2%} (95% bootstrap CI {bm25['all_required_tool_recall']['ci95_low']:.2%}–{bm25['all_required_tool_recall']['ci95_high']:.2%}).", f"- BM25 fail-open rate: {bm25['fail_open_rate']:.2%}; coverage: {bm25['coverage']:.2%}.", "- TF-IDF remains an independently measured baseline; no automatic default swap is made."])
    lines.extend(["", "## Limitations", "", "- This is a synthetic, repository-owned benchmark corpus, not a universal workload claim.", "- Lexical TF-IDF relevance is an offline proxy; no embedding model is downloaded.", "- Selective exposure and lazy hydration are experimental, explicit APIs; the executor contract remains the complete raw schema.", "- Full catalog resend is a modelled protocol assumption, not a claim about every agent framework.", "", "## Reproduction command", "", "```bash", "python -m benchmarks.run --mode fast", "python -m benchmarks.run --mode full", "```"])
    return "\n".join(lines) + "\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--phase4-6", action="store_true", help="Run the explicit optional pinned local semantic benchmark")
    ...
    summary_path.write_text(json.dumps({"schema_version": result["schema_version"], "classification": result["classification"], "dataset": result["dataset"], "summary": result["summary"], "security": result["security"]["status"], "schema_tax": {"catalog_count": result["schema_tax"]["catalog_count"], "tool_schema_count": result["schema_tax"]["tool_schema_count"]}, "phase4": result["phase4"], "phase4_5": result["phase4_5"], "phase4_6": result["phase4_6"]}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
