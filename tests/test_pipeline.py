...
def test_compress_chat_messages_system_prompt_preserved():
    ...
    messages = [
        {"role": "system", "content": "You are a helpful assistant. You must always answer in valid JSON. Never hallucinate."},
        {"role": "user", "content": "Explain machine learning concepts in detail. Machine learning is a branch of AI that focuses on building applications that learn from data. Supervised learning uses labeled data. Unsupervised learning finds patterns in unlabeled data. Reinforcement learning uses reward signals. Deep learning uses neural networks with many hidden layers."}
    ]
...
def test_compress_chat_messages_custom_compressible_roles():
    ...
    compressed = compress_chat_messages(
        messages,
        target_ratio=0.5,
        compressible_roles=["system", "user"]
    )
