...
export function CtaSection() {
  ...
  return (
    <section className="py-24 px-4 sm:px-8 max-w-5xl mx-auto text-center relative overflow-hidden">
      <Card
        glowColor="emerald"
        className="p-8 sm:p-14 space-y-8 relative z-10 overflow-hidden border-emerald-500/40 shadow-[0_0_60px_rgba(0,245,155,0.2)] border-specular-emerald"
      >
        <div className="space-y-4 max-w-2xl mx-auto">
          <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-mono badge-scientific">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-2">
          <div className="flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-[#070A0F] border border-emerald-500/30 font-mono text-sm text-slate-200 shadow-2xl">
            ...
            <button
              onClick={handleCopy}
              className="ml-3 p-1.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-slate-400 hover:text-white transition-all cursor-pointer min-h-[36px]"
            >
              {copied ? (
                <motion.div initial={{ scale: 0.6 }} animate={{ scale: [0.6, 1.25, 1] }}>
